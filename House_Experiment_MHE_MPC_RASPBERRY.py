...
# Questions:
# - Home up and home pumps are reversed :-D
# - Sensibo time seems to be in GMT/UTC, but Tibber is in UTC/GMT+1 any idea why?
# - What are the rules when pulling the "latest" data, or the data for a given date?
# - Is is possible to get the times in a datetime format?
 
# TODO: Replace direct data access via Raspberry pulls
# - Start with Tibber and Sensibo

# TODO:
# - MHE startup, does it help to remove the power weight on past data that do not exist?
# - Fix the code for a startup with all pumps off... The code struggles with that situation
# - Better strategy for pumps off? Can we use the multipliers of target >= 16?
...
PIIP = 'https://6f55c0a34f06.ngrok.io'      # Global (not operational yet)
...
def ReadSettings():
    ...
    return TempSettings, MinTempSettings, WeightMPC, WeightMHE
...
plt.figure(1)
#plt.subplot(1,2,1)
#plt.plot(TibberData['flattime'],marker='o')
#plt.subplot(1,2,2)
plt.step(TibberData['flattime'],np.array(TibberData['Power'])/1e3)#,marker='o')

plt.figure(2)
for index, pump in enumerate(Pumps):
    plt.subplot(2,2,index+1)
    plt.step(DataHP[pump]['flattime'],DataHP[pump]['measurements']['temperature'])
...
"""
response = requests.get(PIIP+'/api/get_data/sensibo/daily/'+str(date.today()))
Sensibo = json.loads(response.text)
Codes = list(Sensibo['data'][str(date.today())].keys())

print('------------------- Sensibo data -------------------')
for pump in Pumps:
    print(pump)
    for code in Codes:
        for time in Sensibo['data'][str(date.today())][code][pump]['time']:
            print(time)
print('----------------------------------------------------')
print('------------------- Tibber data  -------------------')
response = requests.get(PIIP+'/api/get_data/tibber-realtime-home-up/daily/'+str(date.today()))
TibberRT = json.loads(response.text)
Codes = list(TibberRT['data'][str(date.today())].keys())
for code in Codes:
    for time in TibberRT['data'][str(date.today())][code]['time']:
        print(time)
print('----------------------------------------------------')
...
print('Read settings')
TempSettings, MinTempSettings, WeightMPC, WeightMHE = ReadSettings()
...
MHEinputs = struct_symMX([  entry('Perturbation') ])
...
for k in range(N-1):
    ...
    for pump in Pumps:
        ...
        DTtarget      = DataMHE['Data',k,pump,'Ref_temp'] - wMHE['State',k,pump,'Temp']
        DTtarget_plus = DataMHE['Data',k+1,pump,'Ref_temp'] - wMHE['State',k+1,pump,'Temp']
        
        Delta_Power         = wMHE['Param',pump,'Gain']*( DTtarget_plus - DTtarget)/10.  # From power model
        ElecPower_pump_plus = wMHE['State',k,pump,'Power'] + Delta_Power + wMHE['Input',k,pump,'Perturbation']/1e3
        ...
        Delta_Room_Temp =  wMHE['Param',pump,'Room_loss']*DTWallIn/100. + wMHE['Param',pump,'Room_Gain']*PumpPower/1e2
        ...
        J += DataMHE['Weights','Perturbation']*(    wMHE['Input',k,pump,'Perturbation'] )**2/float(N*Npumps)   # Minimize Perturbation
...
"""
TerminalState = struct_symMX([  entry('TargetTemp'),
                                entry('WallTemp'),
                                entry('Power')       ])
"""
...
TerminalStates = []
for pump in Pumps:
    TerminalStates.append( entry(pump, struct = TerminalState) )
TerminalStates = struct_symMX(TerminalStates)
"""
...
for k in range(N-1):
    ...
    for pump in Pumps:
        ...
        g.append(  DataMPC['MinTemperature',k,pump] - wMPC['State',k,pump,'Temp'] - wMPC['Input',k,pump,'SlackMinTemp']   )
        lbgMPC.append(-inf)
        ubgMPC.append(0)
        ...
        J += (DataMPC['Weights','TempBelow']*SlackPenalty + DataMPC['Weights','TempAbove']*(DataMPC['DesiredTemperature',k,pump] - wMPC['State',k,pump,'Temp'])**2)/float(N)
        J += DataMPC['Weights','Delta_TargetTemp']*(HubDeltaTarget**2)*(sqrt( 1 + (wMPC['Input',k,pump,'Delta_TargetTemp']/HubDeltaTarget)**2 ) - 1)/float(N)
        J += (DataMPC['Weights','MinTemp']*wMPC['Input',k,pump,'SlackMinTemp']**2)/float(N)
...
dates = str(date.today()-timedelta(days=1))
response = requests.get(PIIP+'/api/get_data/tibber-realtime-home-up/daily/'+dates)
TibberYesterday = json.loads(response.text)

dates = str(date.today())
response = requests.get(PIIP+'/api/get_data/tibber-realtime-home-up/daily/latest')
TibberToday = json.loads(response.text)

print(TibberYesterday['data']['2021-04-19']['1618853085-22aed73a']['time'][-1])
print(TibberToday['data']['2021-04-20']['1618853085-22aed73a']['time'])
"""

#DataHP, NewHPTime,_ ,_  = PiCall(DataHP, TibberData , TimeInitial, extension = dates)
...
plt.close('all')
plt.figure(1)
plt.step(time_grid,PowerAverage,color='r')
plt.step(TibberData['flattime'],np.array(TibberData['Power'])/1e3)
...
while 0 <= 1:
    ...
    if len(time_grid) >= 0:
        ...
        if MHEWarmStarted:
            for pump in Pumps:
                for key in SYSID[pump].keys():
                    wMHE0['Param',pump,key]  = wMHE_opt['Param',pump,key]
                    lbwMHE['Param',pump,key] = (1-fac)*wMHE_opt['Param',pump,key]
                    ubwMHE['Param',pump,key] = (1+fac)*wMHE_opt['Param',pump,key]
                    DataMHENum[ 'PrevParameters', pump, key ] = wMHE_opt['Param',pump,key]
        else:
            for pump in Pumps:
                for key in SYSID[pump].keys():
        ...
        for k, pump in enumerate(Pumps):
            ...
            #Ax2List[FN][1].step(Times['spot'],np.array(Spot['Prices'])+GridCost,color='r')
        ...
        for pump in Pumps:
            Leg.append(pump)
            AxList[FN][1].step(MHE_time_grid[:-1],wMHE_opt['Input',:,pump,'Perturbation'],color=ColHP[pump])
    ...
    for fig, pump in enumerate(Pumps):
        ...
        for index in range(0,NDisp): #, traj in enumerate(Logger_MHE_MPC['MPC']['RawSolution'][-NMPCtraj:]):
            traj = Logger_MHE_MPC['MPC']['RawSolution'][index]
    ...
    for index in range(0,NDisp): #for index, traj in enumerate(Logger_MHE_MPC['MPC']['RawSolution'][-NMPCtraj:]):
        ...
        for pump in Pumps:
            Traj = wMPC(traj)
            PredPower += np.array(Traj['Input',:NMPCdisphorizon,pump,'On'])*np.array(Traj['State',:NMPCdisphorizon,pump,'Power'])
        AxList[FN][1].step(Logger_MHE_MPC['MPC']['Time_Grid'][index-NDisp][:NMPCdisphorizon],PredPower, where='post', linewidth=1,color='b',linestyle='-')
    ...
    AxList[FN][1].set(title='Total Power [kwh]')
    AxList[FN][1].set_ylim([0,4.5])
    AxList[FN][0].grid()
    AxList[FN][1].grid()
    FigList[FN].canvas.draw()
